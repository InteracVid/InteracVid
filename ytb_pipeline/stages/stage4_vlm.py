"""Stage 4: Visual Language Model Processing

Analyzes video content using vision-language models.
"""
...
def process_single_json(
    ...
    """Process a single JSON file with VLM.
...
def main():
    parser = argparse.ArgumentParser(description="Stage 4: VLM processing for video analysis")
    ...
    parser.add_argument("--start-offset", type=float, default=0.0, help="Start timestamp offset")
    parser.add_argument("--end-offset", type=float, default=0.0, help="End timestamp offset")
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=args.max_workers, thread_name_prefix="VLMProcessor") as executor:
        future_to_task = {
            executor.submit(
                process_single_json,
